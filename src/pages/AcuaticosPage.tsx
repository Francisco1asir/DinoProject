...
import { getDinosaurios, getDinosauriosAcuaticos, getDinosauriosTerrestres, newDino } from '../firebase/FBdinos'
...
export const AcuaticosPage = () => {
  const [dinosaurios, setDinos] = useState<IDinos[]>([])
  useEffect(() => {
    getDinosauriosAcuaticos()
      .then(res => {
        console.log(...res)
        setDinos([...res])
      });
  ...
  return (
    <>
      <ReactPlayer
        ...
        height='100%'
      />
      <h1 id='tittle-home'>Dinosaurios Acuaticos</h1>
        ...
        {dinosaurios
          .map((dino) => (
            <div className='items'>
              ...
              <h1 key={dino.dinoname} id="dinoname">{dino.dinoname}</h1>
